...
import { BestTimeClient } from './client';
...
async function getOptions(): Promise<OptionsData | null> {
  ...
  try {
    const res = await fetch(`${baseUrl}/api/best-time?type=options`, {
      cache: 'no-store',
    });
    ...
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-500">
                Find the fastest months to submit your permit
              </p>
            </div>
          </div>
        </div>
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Suspense fallback={<LoadingState />}>
          {options ? (
            <BestTimeClient categories={options.categories} />
          ) : (
